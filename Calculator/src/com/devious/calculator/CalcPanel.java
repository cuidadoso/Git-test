      ...
      implements ActionListener, Observer
{
	...
	private CalcMode model;
    private JLabel display = new JLabel("0");
    public CalcPanel(CalcMode model)
    {
        super();
        this.model = model;
    ...
    public void actionPerformed(ActionEvent evt)
    {
        ...
        try {
            ...
            case '+':
                model.setOperation(CalcMode.OP_ADD);
                break;
            case '-':
                model.setOperation(CalcMode.OP_SUB);
                break;
            case '*':
                model.setOperation(CalcMode.OP_MUL);
                break;
            case '/':
                model.setOperation(CalcMode.OP_DIV);
                break;
            case '=':
                model.calculate();
                break;
